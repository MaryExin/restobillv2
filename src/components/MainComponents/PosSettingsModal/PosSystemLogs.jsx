...
import { FiSearch, FiDollarSign, FiAward, FiArrowRight, FiActivity, FiTrendingUp, FiLoader, FiCalendar, FiMail } from "react-icons/fi";
...
const PosSystemLogs = ({ isDark, accent }) => {
  ...
  const getPerformanceBadge = (perf) => {
    switch (perf) {
      ...
      case "TRENDING": return { color: "#10b981", bg: "#10b98115", icon: <FiTrendingUp />, label: "Active" };
  ...
  return (
    <div className="space-y-8 text-left duration-700 animate-in fade-in">
      <div className="flex flex-col gap-2 px-2">
        ...
        <p className={`text-[10px] font-black uppercase tracking-[0.4em] ${theme.textMuted}`}>Email-Verified Shift Records</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
          {filteredData.map((staff) => {
            ...
            const profileImg = staff.image 
                ? `${apiHost}/profile_pictures/${staff.image}` 
                : `https://ui-avatars.com/api/?name=${staff.firstname}+${staff.lastname}&background=334155&color=fff&bold=true`;
            ...
            return (
              <div key={staff.id} className={`rounded-[50px] border p-10 space-y-8 group relative transition-all duration-500 hover:translate-y-[-8px] ${theme.panel}`}>
                <div className="flex items-center gap-6">
                   <div className="h-24 w-24 rounded-[30px] p-1 flex items-center justify-center shadow-xl border border-white/10 overflow-hidden bg-slate-800">
                      <img src={profileImg} className="object-cover w-full h-full" alt="profile" />
                   </div>
                ...
                <div className={`p-8 rounded-[35px] border-2 border-dashed transition-all duration-500 ${isDark ? 'border-white/5 bg-black/20' : 'border-slate-100 bg-slate-50'}`}>
                    <div className="grid grid-cols-2 gap-6">
                        <div className="flex flex-col gap-1">
                            <span className={`text-[9px] font-black uppercase tracking-widest opacity-40 ${theme.textPrimary}`}>Sales Count</span>
                        ...
                        <div className="flex flex-col gap-1">
                            <span className={`text-[9px] font-black uppercase tracking-widest opacity-40 ${theme.textPrimary}`}>Total Collected</span>
                            <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between pt-2">
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-[0.2em] ${theme.textMuted}`}>Last Sale Date</span>
                        </div>
                        <span className={`text-[11px] font-black uppercase ${theme.textPrimary}`}>
                            {staff.transaction_date} <span className="mx-1 opacity-30">|</span> {staff.lastTransaction.split(' ')[1] || '00:00'}
                        </span>
                    </div>
