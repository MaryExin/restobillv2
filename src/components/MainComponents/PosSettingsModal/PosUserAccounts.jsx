...
const PosUserAccounts = ({ isDark, accent, getContrastText }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.username || !formData.firstName || !formData.password || !formData.lastName) {
      alert("Please fill all required fields (First Name, Last Name, Email, Password)."); return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData({ 
            firstName: "", middleName: "", lastName: "", 
            username: "", password: "", contact: "", 
            position: "CASHIER", company: businessUnits[0]?.Unit_Name || "" 
        });
      }
      ...
      <div className={`rounded-[50px] border p-12 flex flex-col lg:flex-row items-center justify-between gap-12 ${theme.panel}`}>
        <div className="flex items-center gap-10">
          ...
          <div>
            ...
            <p className={`text-[10px] font-black uppercase tracking-[0.5em] mt-2 ${theme.textMuted}`}>Registry & Employee Sync</p>
          </div>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="px-12 py-6 rounded-[35px] font-black uppercase text-[12px] tracking-[0.3em] hover:scale-105 transition-transform" style={{ backgroundColor: accent, color: contrast }}>
          Add New User
        </button>
      </div>

      {/* TABLE SECTION */}
      <div className={`rounded-[40px] border p-8 overflow-x-auto ${theme.panel}`}>
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.uuid} className="group">
                ...
                <td className={`px-8 py-6 font-black text-[12px] ${theme.subPanel} ${theme.textPrimary}`}>
                    {u.firstname} {u.lastname}
                </td>
                <td className={`px-8 py-6 font-black text-[10px] opacity-60 uppercase tracking-widest ${theme.subPanel} ${theme.textPrimary}`}>
                    {u.position}
                </td>
                <td className={`px-8 py-6 font-black text-[11px] ${theme.subPanel}`} style={{ color: accent }}>
                    {u.company}
                </td>
                <td className={`px-8 py-6 rounded-r-3xl text-right ${theme.subPanel}`}>
                    <button onClick={() => handleDelete(u.uuid, u.firstname)} className="p-3 transition-all text-rose-500 hover:scale-125 hover:rotate-12"><FiTrash2 size={18} /></button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-black/90 backdrop-blur-2xl">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`w-full max-w-6xl rounded-[60px] p-16 overflow-y-auto max-h-[95vh] ${isDark ? 'bg-[#0b1222]' : 'bg-white'}`}>
              <div className="flex items-center justify-between mb-10">
                <h3 className={`text-4xl font-black uppercase tracking-tighter ${theme.textPrimary}`}>User Registration</h3>
                <button onClick={() => setIsModalOpen(false)} className="text-3xl transition-opacity opacity-20 hover:opacity-100"><FiX/></button>
              </div>

              <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
                <div className="flex flex-col items-center lg:col-span-4">
                    <div className="h-72 w-72 rounded-[60px] border-4 border-dashed border-white/10 flex items-center justify-center overflow-hidden bg-white/5 cursor-pointer relative group" onClick={() => fileInputRef.current.click()}>
                        {finalImage ? (
                            <img src={finalImage} className="object-cover w-full h-full" />
                        ) : (
                ...
                <div className="grid grid-cols-1 gap-6 lg:col-span-8 md:grid-cols-2">
                    {[
                        ...
                        <div key={f.id} className="space-y-2 text-left">
                            <label className="text-[9px] font-black uppercase tracking-widest opacity-40 ml-4">{f.label}</label>
                            <input 
                                name={f.id} 
                                type={f.type || 'text'} 
                                value={formData[f.id]} 
                                onChange={(e) => setFormData({...formData, [e.target.name]: e.target.value.toUpperCase()})} 
                    ...
                    <div className="space-y-2 text-left">
                        <label className="text-[9px] font-black uppercase tracking-widest opacity-40 ml-4">Position</label>
                        <select 
                            name="position" 
                    ...
                    <div className="space-y-2 text-left">
                        <label className="text-[9px] font-black uppercase tracking-widest opacity-40 ml-4">Business Unit</label>
                        <select 
                            name="company" 
                            ...
                            className={`w-full p-5 rounded-[25px] border outline-none font-bold text-[12px] ${theme.input}`}
                        >
                            {businessUnits.map(unit => <option key={unit.Unit_Name} value={unit.Unit_Name}>{unit.Unit_Name}</option>)}
                        </select>
                    </div>
                </div>
              </div>
      ...
      {isCropperOpen && (
        <div className="fixed inset-0 z-[2000] bg-black/95 flex flex-col items-center justify-center p-10">
          ...
          <div className="flex gap-4 mt-10">
            <button onClick={() => setIsCropperOpen(false)} className="px-12 py-4 font-black text-[10px] uppercase tracking-widest text-white rounded-2xl bg-white/10 hover:bg-white/20 transition-colors">Cancel</button>
            <button onClick={generateCroppedImage} className="px-16 py-4 font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl" style={{ backgroundColor: accent, color: '#fff' }}>Apply Crop</button>
          </div>
        </div>
      )}
    </div>
