...
const PosCustomerInfo = ({ isDark, accent = "#3b82f6" }) => {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (!response.ok || !result?.success) {
          throw new Error(
            result?.message || "Failed to load customer info settings.",
          );
        }

        if (!cancelled) {
          const nextIsEnabled = Boolean(
            result?.data?.customer_info_enabled || false,
          );
        ...
        console.error(err);
        if (!cancelled) {
          setError(err.message || "Failed to load customer info settings.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Failed to save customer info settings.",
        );
      }
      ...
      setMessage("Customer info settings saved.");
    } catch (err) {
      console.error(err);
      setError(err.message || "Failed to save customer info settings.");
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[10px] font-black tracking-[0.16em] uppercase border border-current/10 bg-white/5">
              ...
              <span style={{ color: accent }}>Discount Customer Details</span>
            </div>
            ...
            <p className={`mt-3 max-w-2xl text-sm ${theme.textMuted}`}>
              Show or hide the Customer Info form in the billing discount modal.
              When enabled, saved details are included with the billing discount
              rows and can load again in payment.
            </p>
          </div>
        ...
        <div className={`mt-6 rounded-2xl border p-4 text-sm ${theme.panelSoft}`}>
          <div className="flex items-center justify-between gap-3">
            ...
            <span className={`text-lg font-black ${theme.textPrimary}`}>
              {initialIsEnabled ? "Enabled" : "Disabled"}
            </span>
          </div>
        </div>
