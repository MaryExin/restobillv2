...
const PricingDashboard = ({ isOpen, onClose }) => {
  ...
  // ── Order Slip config (read-only — managed in Settings › Pricing & Order Slip) ─
  ...
  }, [apiHost, selectedService, activeCategory]);
    ...
    fetch(`${apiHost}/api/pos_order_slip_settings.php`)
      ...
      .then((data) => {
        if (!cancelled && data.success) {
          setNoPriceOnOS(Boolean(data.data?.hide_price_on_os));
          setGroupByCategory(Boolean(data.data?.group_by_category));
        }
      })
      .catch(() => {});
  ...
  // When "group by category" is ON, sort grid items by category then name
      ...
      <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/60 backdrop-blur-md p-6 font-sans overflow-hidden text-slate-700">
        ...
        >
          ...
          <div className="p-6 border-b border-slate-50">
            ...
            <div className="flex items-center justify-between gap-4 mb-5">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ backgroundColor: COLORS.brandLighter, color: COLORS.brand }}
                >
                  <Tag size={18} />
                </div>
                <div>
                  ...
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                    Terminal: <span style={{ color: COLORS.brand }}>{selectedService}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 text-white rounded-2xl bg-blue-600 shadow-lg shadow-blue-600/20 hover:bg-blue-700 transition-colors"
                >
                  <Plus size={16} />
                  <span className="text-[11px] font-black uppercase">New Product</span>
                </button>

                <select
                  value={selectedService}
                  onChange={(e) => setSelectedService(e.target.value)}
                  className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-[11px] font-black outline-none"
                >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="py-2.5 pl-9 pr-4 bg-slate-50 rounded-xl border text-[11px] font-bold outline-none w-48 focus:w-64 transition-all"
                  />
                </div>
            ...
            {/* Row 2 — Category tabs (synced to product data) */}
          ...
          <div className="flex-1 p-8 overflow-y-auto bg-[#fcfdfe] no-scrollbar">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center mt-20 gap-3 text-slate-300">
              ...
              >
                {filteredProducts.map((item, idx) => (
                  <motion.div
                    layout
                    key={idx}
                    className="relative flex flex-col overflow-hidden rounded-[2.2rem] border border-slate-100 bg-white shadow-sm group hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
                  >
                    {enablePictures && (
                      <div className="relative w-full overflow-hidden aspect-square bg-slate-50">
                        <img
                          ...
                          onError={(e) => {
                            e.target.src = `${apiHost}/item_pictures/default.jpg?t=${new Date().getTime()}`;
                          }}
                    ...
                    <div className="flex flex-col justify-between flex-1 p-5">
                      <div>
                        ...
                        <h3 className="text-[11px] font-black text-slate-700 leading-tight uppercase line-clamp-2">
                          {item.item_description}
                        </h3>

                        {/* Category badge shown when group-by-category is active */}
                        {groupByCategory && (
                          <span className="inline-block mt-1.5 px-2 py-0.5 text-[8px] font-black uppercase rounded-lg bg-blue-50 text-blue-500 border border-blue-100 tracking-wide">
                      ...
                      <div className="flex items-end justify-between mt-4">
                        {/* Price hidden badge when noPriceOnOS is active */}
                        {noPriceOnOS ? (
                          <div className="flex items-center gap-1.5">
                            <p className="text-[18px] font-black text-blue-600">
                              ₱{Number(item.srp).toLocaleString()}
                            </p>
                            <span className="text-[8px] font-black text-slate-300 uppercase bg-slate-50 border border-slate-100 rounded-lg px-2 py-0.5 leading-none">
                              Hidden on OS
                            </span>
                          </div>
                        ) : (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-2.5 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors shadow-sm border border-blue-100"
                          >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mt-10">
                ...
                <button
                  ...
                  className="py-4 text-white text-[10px] font-black uppercase rounded-[1.2rem] bg-blue-600 shadow-lg shadow-blue-600/20"
                >
                  Save Entry
                </button>
              </div>
              ...
              {!showConfirm ? (
                <>
                  ...
                  <button
                    ...
                    className="w-full py-4 text-white text-[11px] font-black uppercase rounded-2xl shadow-lg bg-blue-600"
                  >
                    Update Price
                  </button>
                  <button
                ...
                <div className="py-2">
                  <div className="flex items-center justify-center w-16 h-16 mx-auto mb-6 text-blue-600 rounded-full bg-blue-50">
                    <AlertCircle size={32} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-8">
                    ...
                    <button
                      ...
                      className="py-4 text-white text-[10px] font-black rounded-xl uppercase bg-blue-600 shadow-lg shadow-blue-600/20"
                    >
                      Confirm
                    </button>
                  </div>
