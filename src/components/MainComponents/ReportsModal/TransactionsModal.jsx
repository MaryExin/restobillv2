      ...
      <div className={`flex items-center justify-between px-1 mb-4 ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>
        ...
        <div className="flex gap-1">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setCurrentView(new Date(currentView.getFullYear(), currentView.getMonth() - 1));
            }}
            ...
            <button
              ...
              className={`h-8 w-full rounded-lg font-bold text-[10px] transition-all ${
                dateString === selectedDate
                  ...
                  : isDark ? "text-slate-400 hover:bg-white/10" : "text-slate-600 hover:bg-blue-50 hover:text-blue-600"
              }`}
            >
              {day}
            </button>
      ...
      <div className={`w-full max-w-lg border rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[90vh] transition-all ${isDark ? "bg-[#0a0f1e] border-white/10" : "bg-white border-slate-200"}`}>
        ...
        <div className={`p-8 border-b flex justify-between items-center ${isDark ? "bg-white/[0.02] border-white/5" : "bg-slate-50 border-slate-100"}`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className={`text-xl font-black leading-none tracking-tighter uppercase ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>Order <span className="text-blue-500">Items</span></h3>
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Ref: #{transaction.transaction_id}</p>
            </div>
          </div>
          <button onClick={onClose} className={`flex items-center justify-center w-10 h-10 transition-all rounded-xl ${isDark ? "bg-white/5 text-slate-500 hover:bg-rose-600 hover:text-white" : "bg-slate-200 text-slate-500 hover:bg-rose-600 hover:text-white shadow-sm"}`}>
            <FaTimes />
          </button>
        </div>
            ...
            <div className="space-y-3">
              {items.map((item, i) => (
                <div key={i} className={`flex justify-between items-center p-4 rounded-2xl border transition-all ${isDark ? "bg-white/[0.02] border-white/5" : "bg-slate-50/50 border-slate-100"}`}>
                  <div className="flex items-center gap-4">
                    <div className={`h-9 w-9 rounded-xl flex items-center justify-center text-[11px] font-black border ${isDark ? "bg-white/5 text-blue-400 border-white/5" : "bg-white text-blue-600 border-slate-200 shadow-sm"}`}>{item.sales_quantity}x</div>
                    <div>
                      <p className={`text-sm font-black uppercase ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>{item.item_name}</p>
                      <p className="text-[10px] text-slate-500 font-bold">₱{parseFloat(item.selling_price).toLocaleString()}</p>
                    </div>
                  </div>
                  <p className={`text-sm font-black ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>₱{parseFloat(item.subtotal).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                </div>
              ))}
            </div>
          ) : (
            <div className="py-20 text-xs font-black tracking-widest text-center uppercase text-slate-400">No records found</div>
          )}
        </div>
            ...
            {payments.length > 0 ? (
              payments.map((p, idx) => (
                <div key={idx} className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-black ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>
                    ₱{parseFloat(p.payment_amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </span>
                </div>
              ))
            ) : (
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-black ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>
                  ₱{parseFloat(transaction.TotalAmountDue).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </span>
              </div>
            )}
          </div>

          <div className="flex items-end justify-between pt-4 border-t border-slate-200/50">
             <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Grand Total</p>
                <div className="flex gap-4 text-[9px] font-black uppercase text-rose-500">
                   <span>Disc: -₱{parseFloat(transaction.Discount || 0).toLocaleString()}</span>
                </div>
             </div>
             <div className="text-right">
                <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mb-1">Total Paid</p>
                <h4 className={`text-3xl font-black leading-none tracking-tighter ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>
                  ₱{parseFloat(transaction.TotalAmountDue).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </h4>
             </div>
          </div>
...
const TransactionsModal = ({ isOpen, onClose }) => {
  ...
  const paymentBreakdown = useMemo(() => {
    return data.reduce(
      (acc, item) => {
        const amount = parseFloat(item.TotalAmountDue || 0);
  ...
  return (
    <div className={`fixed inset-0 z-[100000] flex items-center justify-center backdrop-blur-md p-2 font-sans overflow-hidden transition-all duration-300 ${isDark ? "bg-[#020617]/95" : "bg-slate-500/40"}`}>
      <div className={`relative flex h-[98vh] w-full max-w-[99%] flex-col overflow-hidden rounded-[2.5rem] border shadow-2xl transition-all duration-300 ${isDark ? "bg-[#020617] border-white/10" : "bg-[#f8fafc] border-white"}`}>
        ...
        <div className={`flex items-center justify-between px-10 py-6 border-b shrink-0 transition-all ${isDark ? "border-white/5" : "bg-white border-slate-200"}`}>
          <div className="flex items-center gap-6">
            <div className={`h-12 w-12 rounded-2xl flex items-center justify-center shadow-lg ${isDark ? "bg-[#3b82f6]/10 text-[#3b82f6] shadow-blue-500/10" : "bg-blue-50 text-blue-600 shadow-blue-100"}`}>
              <FaHistory size={22} />
            </div>
            <div>
              <h2 className={`text-3xl font-black leading-none tracking-tighter uppercase ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>Transaction <span className="text-[#3b82f6]">Records</span></h2>
              <p className="mt-1 text-[9px] font-black uppercase text-slate-500 tracking-widest">Branch Database | CNC System</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={exportExcel} className={`h-11 px-6 rounded-xl border transition-all font-black text-[10px] uppercase tracking-widest flex items-center gap-2 ${isDark ? "bg-white/5 text-slate-300 border-white/10 hover:bg-white/10" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50 shadow-sm"}`}>
              <FaFileExcel size={14} className="text-emerald-500" /> Export
            </button>
            <button onClick={onClose} className={`flex items-center justify-center transition-all border h-11 w-11 rounded-xl ${isDark ? "bg-white/5 text-white hover:bg-rose-600 border-white/10" : "bg-white text-slate-400 hover:text-rose-600 border-slate-200 shadow-sm"}`}>
              <FaTimes size={18} />
            </button>
          </div>
        </div>
          ...
          <div className={`h-full overflow-auto rounded-3xl border relative custom-scrollbar transition-all ${isDark ? "bg-[#050a18] border-white/5" : "bg-white border-slate-200 shadow-inner"}`}>
            <table className="w-full text-left border-separate border-spacing-0 min-w-[1700px]">
              <thead className="sticky top-0 z-[50]">
                <tr className={`${isDark ? "bg-[#0a0f1e]/95 text-slate-500" : "bg-slate-50 text-slate-400"} backdrop-blur-md text-[9px] font-black uppercase tracking-widest`}>
                  <th className={`p-5 sticky left-0 z-[60] border-b ${isDark ? "bg-[#0a0f1e] border-white/5" : "bg-slate-50 border-slate-200"}`}>ID</th>
                  <th className="p-5 border-b">Invoice / Slip</th>
                  <th className="p-5 text-center border-b">Timestamp</th>
                  <th className="p-5 border-b">Table</th>
                  <th className="p-5 text-right border-b">Gross</th>
                  <th className="p-5 text-right border-b text-rose-500">Disc.</th>
                  <th className={`p-5 border-b text-right ${isDark ? "bg-white/[0.01]" : "bg-blue-50/30"}`}>Net Due</th>
                  <th className="p-5 text-center border-b">Method</th>
                  <th className="p-5 text-center border-b">Status</th>
                  <th className="p-5 text-center border-b">View</th>
                </tr>
              </thead>
              <tbody className={`divide-y font-bold transition-all ${isDark ? "divide-white/[0.03] text-slate-400" : "divide-slate-100 text-slate-600"}`}>
                {data.map((row, idx) => (
                  <tr key={idx} className={`transition-colors text-[13px] ${isDark ? "hover:bg-white/[0.02]" : "hover:bg-blue-50/50"}`}>
                    ...
                    <td className="p-5">
                      <div className={isDark ? "text-white" : "text-[#2e4a7d]"}>INV: {row.invoice_no || "---"}</div>
                      <div className="text-[10px] text-slate-500 uppercase font-medium">Slip: {row.order_slip_no || "---"}</div>
                    </td>
                    ...
                    <td className="p-5 uppercase">{row.table_number || "Take-out"}</td>
                    <td className="p-5 text-right">₱{parseFloat(row.TotalSales).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                    <td className="p-5 font-black text-right text-rose-500/60">-{parseFloat(row.Discount || 0).toLocaleString()}</td>
                    <td className={`p-5 text-right text-xl font-black tracking-tighter transition-all ${isDark ? "text-white bg-white/[0.01]" : "text-[#2e4a7d] bg-blue-50/30"}`}>₱{parseFloat(row.TotalAmountDue).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                    <td className="p-5 text-center">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase ${row.payment_method?.toLowerCase() === "cash" ? "bg-emerald-500/10 text-emerald-500" : "bg-blue-500/10 text-blue-500"}`}>{row.payment_method}</span>
                    </td>
                    <td className="p-5 text-center">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase shadow-sm ${row.status === "Paid" ? "bg-emerald-600 text-white" : "bg-rose-600 text-white"}`}>{row.status}</span>
                    </td>
                    <td className="p-5 text-center">
                      <button onClick={() => setViewingTransaction(row)} className={`h-10 w-10 flex items-center justify-center rounded-xl transition-all shadow-inner mx-auto ${isDark ? "bg-white/5 text-blue-500 hover:bg-[#3b82f6] hover:text-white" : "bg-slate-100 text-blue-600 hover:bg-blue-600 hover:text-white"}`}>
                        <FaEye size={16} />
                      </button>
                    </td>
                  </tr>
        ...
        {/* FOOTER STATS */}
        <div className={`flex items-center justify-between px-10 py-6 border-t shrink-0 transition-all ${isDark ? "bg-black/40 border-white/5" : "bg-white border-slate-200"}`}>
          <div className="flex gap-10">
            <div>
              <p className={`text-[8px] font-black uppercase tracking-widest mb-1 ${isDark ? "text-slate-600" : "text-slate-400"}`}>Transactions</p>
              <p className={`text-2xl font-black leading-none ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>{data.length}</p>
            </div>
            <div className={`h-8 w-[1px] self-center ${isDark ? "bg-white/5" : "bg-slate-200"}`}></div>
            <div>
              <p className="text-[8px] font-black uppercase tracking-widest mb-1 text-emerald-500">Cash Total</p>
              <p className={`text-2xl font-black ${isDark ? "text-emerald-400" : "text-emerald-600"}`}>₱{paymentBreakdown.cash.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            <div className={`h-8 w-[1px] self-center ${isDark ? "bg-white/5" : "bg-slate-200"}`}></div>
            <div>
              <p className="text-[8px] font-black uppercase tracking-widest mb-1 text-blue-500">GCash Total</p>
              <p className={`text-2xl font-black ${isDark ? "text-blue-400" : "text-blue-600"}`}>₱{paymentBreakdown.gcash.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
          </div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Live Database Synced</p>
        </div>

        {/* SIDEBAR FILTER */}
        {showFilter && (
          <div className={`absolute inset-y-0 right-0 w-[400px] border-l z-[100001] p-10 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300 transition-all ${isDark ? "bg-[#0a0f1e] border-white/10" : "bg-white border-slate-200"}`}>
            <div className="flex items-center justify-between mb-10 shrink-0">
              <h3 className={`text-2xl font-black tracking-tighter uppercase ${isDark ? "text-white" : "text-[#2e4a7d]"}`}>Advanced <span className="text-[#3b82f6]">Filter</span></h3>
            ...
            <div className="flex-1 pr-2 space-y-10 overflow-y-auto custom-scrollbar">
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 block ml-1">Payment Method / Type</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: "All", icon: <FaListUl />, label: "Show All" },
                    { id: "Active", icon: <FaCheckCircle />, label: "Paid Only" },
                    { id: "Void", icon: <FaBan />, label: "Voided" },
                    { id: "Refund", icon: <FaUndo />, label: "Refunds" },
                  ].map((s) => (
                    <button
                      key={s.id}
                      onClick={() => setStatusFilter(s.id)}
                      className={`flex flex-col items-start p-4 rounded-2xl border transition-all ${statusFilter === s.id ? "bg-[#3b82f6] border-[#3b82f6] text-white shadow-lg" : isDark ? "bg-white/5 border-white/5 text-slate-500 hover:border-white/10" : "bg-slate-50 border-slate-100 text-slate-400 hover:border-blue-200"}`}
                    >
