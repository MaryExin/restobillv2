...
const MonthlySalesModal = ({ isOpen, onClose }) => {
  ...
  const fetchSales = useCallback(async () => {
    ...
    try {
      ...
      setSalesData(result?.monthlySales || []);
    } catch (err) {
      console.error(err);
      setSalesData([]);
    } finally {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), "Monthly Sales");
    XLSX.writeFile(wb, `Monthly_Sales_${dateFrom}_to_${dateTo}.xlsx`);
  };
      ...
      <div className={`relative flex h-[97vh] w-full max-w-[99%] flex-col overflow-hidden rounded-[32px] border shadow-[0_30px_90px_rgba(15,23,42,0.18)] ${isDark ? "border-white/10 bg-[#020617]" : "border-slate-200 bg-[#f8fafc]"}`}>
        ...
        <div className={`shrink-0 border-b px-8 py-6 ${isDark ? "border-white/10 bg-white/[0.03]" : "border-slate-200 bg-white"}`}>
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className={`text-sm font-semibold ${isDark ? "text-purple-400" : "text-purple-600"}`}>Reports Dashboard</div>
              <h2 className={`mt-1 text-3xl font-bold sm:text-4xl ${isDark ? "text-white" : "text-slate-900"}`}>Monthly Sales</h2>
              <div className="flex gap-4 mt-4">
                {[
                  { label: "Total Gross",     val: totals.gross, cls: isDark ? "text-white" : "text-slate-900" },
                  { label: "Total Discounts", val: totals.disc,  cls: "text-rose-500",    prefix: "-" },
                  { label: "VAT Collected",   val: totals.vat,   cls: "text-emerald-500" },
                ].map(({ label, val, cls, prefix = "" }) => (
                  <div key={label} className={`px-4 py-2 rounded-2xl border ${isDark ? "bg-white/5 border-white/10" : "bg-white border-slate-200"}`}>
                    <p className={`text-[10px] uppercase tracking-wider font-bold ${cls}`}>{label}</p>
                    <p className={`text-lg font-bold ${cls}`}>{prefix}{peso(val)}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className={`flex shrink-0 items-center gap-4 px-8 py-4 ${isDark ? "bg-[#050a18]/40" : "bg-slate-100/70"}`}>
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by period (e.g. June 2025)..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className={`h-12 w-full rounded-2xl border pl-11 pr-4 text-sm font-medium outline-none transition ${isDark ? "border-white/10 bg-[#0a0f1e] text-white placeholder:text-slate-500 focus:border-purple-500/50" : "border-slate-200 bg-white text-slate-700 placeholder:text-slate-400 focus:border-purple-500"}`}
            />
          </div>
          ...
          <button onClick={() => setShowFilter(true)} className="flex h-12 items-center gap-2 px-5 text-sm font-semibold text-white transition bg-purple-600 rounded-2xl hover:bg-purple-700">
            ...
            <table className="min-w-[3000px] w-full border-separate border-spacing-0 text-left">
              <thead className="sticky top-0 z-[50]">
                <tr className={`text-[11px] font-bold uppercase tracking-wider ${isDark ? "bg-[#0a0f1e] text-slate-500" : "bg-slate-50 text-slate-400"}`}>
                  <th className={`sticky left-0 top-0 z-[60] border-b p-4 ${isDark ? "border-white/10 bg-[#0a0f1e]" : "border-slate-200 bg-slate-50"}`}>Period</th>
              ...
              <tbody className={`divide-y ${isDark ? "divide-white/5 text-slate-300" : "divide-slate-100 text-slate-700"}`}>
                {filtered.length > 0 ? filtered.map((item, idx) => (
                  <tr key={idx} className={`text-sm transition ${isDark ? "hover:bg-purple-500/5" : "hover:bg-purple-50"}`}>
                    <td className={`sticky left-0 z-[40] border-r p-4 font-bold ${isDark ? "border-white/5 bg-[#050a18] text-purple-400" : "border-slate-100 bg-white text-purple-600"}`}>
                      {item.Period}
                    </td>
                  ...
                  <tr>
                    <td colSpan={18} className={`p-10 text-center text-sm ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                      No monthly sales data found for the selected period.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className={`flex shrink-0 items-center justify-between border-t px-8 py-5 ${isDark ? "border-white/10 bg-black/20" : "border-slate-200 bg-white"}`}>
          <div className="flex items-center gap-8">
            <div>
              <p className={`text-xs font-medium ${isDark ? "text-slate-500" : "text-slate-400"}`}>Reporting Months</p>
          ...
          <div className="px-8 py-4 text-right bg-purple-600 shadow-xl rounded-2xl">
        ...
        {showFilter && (
          <div className={`absolute inset-y-0 right-0 z-[100001] flex w-[420px] flex-col border-l p-8 shadow-2xl animate-in slide-in-from-right duration-300 ${isDark ? "border-white/10 bg-[#0a0f1e]" : "border-slate-200 bg-white"}`}>
                ...
                <div className={`grid grid-cols-1 gap-2 rounded-2xl p-1 ${isDark ? "bg-black/20" : "bg-slate-100"}`}>
                  {["Active", "Voided", "All"].map((s) => (
                    <button key={s} onClick={() => setStatus(s)}
                      className={`h-11 rounded-xl text-sm font-medium transition ${status === s ? "bg-purple-600 text-white shadow-lg" : isDark ? "text-slate-400 hover:bg-white/10 hover:text-white" : "bg-white text-slate-600 hover:text-purple-600"}`}>
                      {s}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={() => { fetchSales(); setShowFilter(false); }}
              className="mt-8 rounded-2xl bg-purple-600 py-4 text-sm font-semibold text-white shadow-lg shadow-purple-500/20 transition hover:bg-purple-700 active:scale-[0.98]">
