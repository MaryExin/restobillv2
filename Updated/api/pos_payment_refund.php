...
try {
    ...
    // ── Get next refund number and save ───────────────────────────────────────
    ...
    if ((int)($transaction["refund_id"] ?? 0) > 0) {
        $pdo->rollBack();
        respond(false, "Transaction is already refunded.", 409);
    }

    if ((int)($transaction["void_id"] ?? 0) > 0) {
        $pdo->rollBack();
        respond(false, "A voided transaction cannot be refunded.", 409);
    }

    $counterStmt = $pdo->prepare("
        SELECT next_refund_id
    ...
    $refundNumber = (int)$counter["next_refund_id"];
    ...
    respond(true, "Transaction refunded successfully.", 200, [
    ...
    error_log("POS refund failed: " . $e->getMessage());
    respond(false, "Unable to refund the transaction.", 500);
}
