...
class ProductSyncMutateGateway
{
    ...
    public function syncSelectedRows(array $data, int|string $userId): array
    {
        ...
        try {
            ...
            [$syncedSalesTypes, $syncedSalesTypeMappings] = $this->syncOfflineSalesTypesAndPricingMappings(
                $busunitCode,
                $userId
            );
    ...
    private function syncOfflineSalesTypesAndPricingMappings(int|string $busunitCode, int|string $userId): array
    {
        $webSalesTypes = $this->getWebSalesTypes();
        $webMappings = $this->getWebPricingBySalesTypePerBu((string) $busunitCode);

        $this->deleteAllOfflinePricingBySalesType();
        $this->deleteAllOfflineSalesTypes();

        $salesTypeCount = $this->insertOfflineSalesTypes($webSalesTypes);
        $mappingCount = $this->insertOfflinePricingBySalesTypeMappings($webMappings, $userId);

        return [$salesTypeCount, $mappingCount];
    }

    private function getWebSalesTypes(): array
    {
        $stmt = $this->webConn->query("
            SELECT
                seq,
                sales_type_id,
                description,
