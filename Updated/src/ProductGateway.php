...
class ProductGateway
{
    ...
    public function getbyPageData($pageIndex, $pageData)
    {
        ...
        $sql = "SELECT * FROM lkp_products ORDER BY seq LIMIT $pageIndex, $pageData";
    ...
    public function createForUser($user_id, $data)
    {
        ...
        $sql = "INSERT INTO lkp_products ()



                VALUES (default, CONCAT('PD-',ShortUUID()), :desc, :product_qty, :uomval, :uom, :cost, :srp, 'Active', :user_tracker, DATE_ADD(NOW(), INTERVAL 8 HOUR))";

        $stmt = $this->conn->prepare($sql);

        $stmt->bindValue(":desc", $data["desc"], PDO::PARAM_STR);

        $stmt->bindValue(":product_qty", $data["product_qty"], PDO::PARAM_STR);
