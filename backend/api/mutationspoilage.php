...
$gateway = new ProductsQueueGateway($database);

//Initialize Task Controller to process API Requests

$controller = new ProductsQueueController($gateway, $user_id);

if ($queueid === null) {

    $controller->processSpoilage($_SERVER['REQUEST_METHOD'], $data);

} else {

    $controller->processPatchRequest($_SERVER['REQUEST_METHOD'], $data, $queueid);
