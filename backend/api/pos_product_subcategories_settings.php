...
function readSetting(PDO $pdo): bool
{
    ...
    ");
    $stmt->execute([":category" => SUBCATEGORIES_CATEGORY, ":description" => SUBCATEGORIES_DESCRIPTION]);
...
function saveSetting(PDO $pdo, bool $enabled): void
{
    $existingStmt = $pdo->prepare("
        ...
        ORDER BY ID DESC LIMIT 1
    ");
    $existingStmt->execute([":category" => SUBCATEGORIES_CATEGORY, ":description" => SUBCATEGORIES_DESCRIPTION]);
    ...
    if ($existingId) {
        ...
        $pdo->prepare("INSERT INTO tbl_pos_settings (ID, category, description, `value`) VALUES (:id, :cat, :desc, :v)")
            ->execute([":id" => $nextId, ":cat" => SUBCATEGORIES_CATEGORY, ":desc" => SUBCATEGORIES_DESCRIPTION, ":v" => $val]);
    }
}
...
try {
    ...
    if ($method === "GET") {
        respond(true, "Product subcategories settings loaded.", ["enable_subcategories" => readSetting($pdo)]);
    }
    ...
    if (array_key_exists("enable_subcategories", $body)) {
        saveSetting($pdo, normalizeBoolean($body["enable_subcategories"]) === "True");
    }
