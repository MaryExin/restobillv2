...
class SLGateway
{
    ...
    {
        $checkforduplisql = "SELECT COUNT(*) as count FROM lkp_slcodes WHERE glcode = :glcodes &&
        slcodes = :slcodes &&  deletestatus = 'Active' ||
        sldescription = :sldescriptions && deletestatus = 'Active'";
        ...
        $checkforduplistmt->bindValue(":sldescriptions", $data["sldescription"], PDO::PARAM_STR);
        ...
        $sql = "INSERT INTO lkp_slcodes ()
                ...
                VALUES (CONCAT('SL-',ShortUUID()),:glcodes,:slcodes ,:sldescriptions , 'Active', :user_tracker, DATE_ADD(NOW(), INTERVAL 8 HOUR))";

        $stmt = $this->conn->prepare($sql);

        $stmt->bindValue(":glcodes", $data["glcode"], PDO::PARAM_STR);
        $stmt->bindValue(":slcodes", $data["slcode"], PDO::PARAM_STR);
        $stmt->bindValue(":sldescriptions", $data["sldescription"], PDO::PARAM_STR);
