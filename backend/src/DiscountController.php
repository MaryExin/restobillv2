    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $discountid = join($discountcode);

                        $rows = $this->gateway->rejectdiscounts($this->user_id,$discountid);
                        // echo json_encode(["message" => "Discount deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editdiscount($this->user_id,$data);
                        // echo json_encode(["message" => "Discount Edit", "rows" => $rows]);
                }
            } else {
            ...
            switch ($method) {
                ...
                case "PATCH":
                        $discountcode = $data["brand_code"];

                        $rows = $this->gateway->rejectdiscounts($data);
