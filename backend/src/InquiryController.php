    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $inquirycode = $data["Inquiry_code"];
                $inquiryid = join($inquirycode);

                        $rows = $this->gateway->rejectinquirys($this->user_id,$inquiryid);
                        echo json_encode(["message" => "Inquiry deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editinquiry($this->user_id,$data);
                        echo json_encode(["message" => "Inquiry Edit", "rows" => $rows]);
                }
            } else {
            ...
            switch ($method) {
                ...
                case "PATCH":
                        $inquirycode = $data["brand_code"];

                        $rows = $this->gateway->rejectinquirys($data);
