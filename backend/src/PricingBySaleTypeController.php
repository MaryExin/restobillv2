...
class PricingBySaleTypeController
{
    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $pricingbysaletypecode = $data["Pricingbysaletypecode"];
                $pricingbysaletypeid = join($pricingbysaletypecode);

                        $rows = $this->gateway->rejectpricingbysaletype($this->user_id,$pricingbysaletypeid);
                        // echo json_encode(["message" => "Product Category deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editpricingbysaletype($this->user_id,$data);
                        // echo json_encode(["message" => "Product Category Edit", "rows" => $rows]);
                }
            } else {
