    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                // $mopcode = $data["mop_code"];
                // $mopid = join($mopcode);

                        $rows = $this->gateway->rejectmops($this->user_id,$data);
                        // echo json_encode(["message" => "mop deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editmop($this->user_id,$data);
