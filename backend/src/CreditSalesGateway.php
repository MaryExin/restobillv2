...
class CreditSalesGateway
{
    ...
    public function getSalesInvoice($data)
    {
        ...
        $sql = "SELECT tbl_mop_summary.transdate, tbl_mop_summary.mop_id, lkp_mop.description,
                IF((tbl_mop_summary.amount) -  SUM(tbl_credit_sales_clearing.amount_cleared * -1) is null ,
                tbl_mop_summary.amount,(tbl_mop_summary.amount) -  SUM(tbl_credit_sales_clearing.amount_cleared * -1)) as amount
                 , tbl_mop_summary.payment_ref,
                tbl_mop_summary.sales_id, tbl_mop_summary.payment_status
                FROM tbl_mop_summary LEFT OUTER JOIN lkp_mop ON tbl_mop_summary.mop_id = lkp_mop.mop_id
                LEFT JOIN tbl_credit_sales_clearing ON tbl_credit_sales_clearing.clearing_reference = tbl_mop_summary.sales_id
                WHERE tbl_mop_summary.deletestatus = 'Active'
                AND tbl_mop_summary.sales_id = :salesid
                UNION
                SELECT 	subquery.transdate,'',payment_type,subquery.description ,'',subquery.clearing_reference ,'Paid' 
				FROM (SELECT transdate, payment_type,SUM(amount_cleared * -1) as description ,'',clearing_reference
    ...
    public function clearForUser($user_id, $data)
    {
        ...
        try {
            ...
            $stmt->bindValue(":customername", 'ACCOUNT RECEIVABLE - ' . $data["customername"], PDO::PARAM_STR);
            ...
            if ($stmt->rowCount() > 0) {
                ...
                VALUES (default, DATE(DATE_ADD(NOW(), INTERVAL 8 HOUR)),
                    DATE(DATE_ADD(NOW(), INTERVAL 8 HOUR)), :glcode, :slcode,
