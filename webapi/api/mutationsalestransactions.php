...
$gateway = new SalesTransactionsGateway($database);

//Initialize Task Controller to process API Requests

$controller = new SalesTransactionsController($gateway, $user_id);

if ($salesTransId === null) {
    $controller->processRequest($_SERVER['REQUEST_METHOD'], $data);
} else {
    $controller->processPatchRequest($_SERVER['REQUEST_METHOD'], $data, $salesTransId);
}
