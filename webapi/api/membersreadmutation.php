...
if (!array_key_exists("pageItems", $data)) {
    $pageData = null;
}else{
    $pageData =  intval($data["pageItems"]); 
}
...
// echo json_encode(["message_1" =>   $memberId . " " . $pageData . " " . $pageIndex ]);
...
$membersReadGateway = new MembersReadGateway($database);

//Initialize Task Controller to process API Requests
$controller = new MembersReadController($membersReadGateway, $user_id);

$controller->processRequest($_SERVER['REQUEST_METHOD'], $pageIndex, $pageData, $memberId);
