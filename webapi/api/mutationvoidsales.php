...
$gateway = new SalesSummaryGateway($database);

//Initialize Task Controller to process API Requests

$controller = new SalesSummaryController($gateway, $user_id);

if ($data["type"] === "cashier") {
    $controller->processCashiersVoidRequest($_SERVER['REQUEST_METHOD'], $data);
} else {
    $controller->processVoidRequest($_SERVER['REQUEST_METHOD'], $data);
}
