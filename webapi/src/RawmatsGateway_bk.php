...
class RawmatsGateway
{
    ...
    public function createForUser($user_id, $data)
    {
        ...
        $checkSQL = "SELECT COUNT(*) FROM lkp_raw_mats WHERE productcode = :productcode AND deletestatus = 'Active'";
        $stmt = $this->conn->prepare($checkSQL);
        $stmt->bindValue(":productcode", $data["productcode"]);
        $stmt->execute();
    ...
    // Check for duplicate entry for productcode
    $checkSQL = "SELECT COUNT(*) FROM lkp_raw_mats  
                WHERE productcode = :productcode
                AND seq <> :seq 
                AND deletestatus = 'Active'";
    $stmt = $this->conn->prepare($checkSQL);
    $stmt->bindValue(":productcode", $id["productcode"]);
    $stmt->bindValue(":seq", $id["seq"]);
    $stmt->execute();
   ...
   $checkforduplisql = "SELECT COUNT(*) as count FROM lkp_raw_mats 
                        WHERE `desc` = :rawmatsdescs 
                        AND seq <> :seq 
                        AND deletestatus = 'Active'";
    ...
    $checkforduplistmt->bindValue(":seq", $id["seq"]);
    $checkforduplistmt->execute();
    $rowCount = $checkforduplistmt->fetchColumn();
    ...
    if($rowCount > 0) {
        ...
        $stmt->bindValue(":productcode", $id["productcode"], PDO::PARAM_STR);
    ...
    public function getInfiniteReadData($page, $pageIndex, $pageData, $search)
    {
        ...
        $sql = "SELECT
                    lkp_raw_mats.seq, 
                    lkp_raw_mats.mat_code,
                    lkp_raw_mats.desc,
                    lkp_raw_mats.mat_qty,
                    lkp_raw_mats.uomval,
                    lkp_raw_mats.uom,
                    tbl_pricing_details.pricing_code AS pricing_category,
                    lkp_pricing_code.pricing_code,
                    tbl_pricing_details.cost_per_uom,
                    lkp_raw_mats.level,
                    lkp_raw_mats.expiry_days,
                    lkp_raw_mats.rawmats_parent,
                    lkp_raw_mats.category,
                    tbl_pricing_details.srp,
                    lkp_raw_mats.brandcode,
                    lkp_product_brand.brand_desc,
                    lkp_raw_mats.productcode,
                    lkp_raw_mats.deletestatus 
                FROM
                    lkp_raw_mats
                        LEFT OUTER JOIN
                    tbl_pricing_details ON lkp_raw_mats.mat_code = tbl_pricing_details.inv_code
                        LEFT OUTER JOIN
                    lkp_pricing_code ON tbl_pricing_details.pricing_code = lkp_pricing_code.uuid
                            LEFT OUTER JOIN
                    lkp_product_brand ON lkp_raw_mats.brandcode = lkp_product_brand.brandcode
                    WHERE lkp_raw_mats.deletestatus = 'Active'
            AND (
                lkp_raw_mats.desc LIKE :search
                OR lkp_raw_mats.productcode LIKE :codesearch
            )
