...
class ClosingAccountingPeriodGateway
{
    ...
    public function closeAccountingEntries($user_id, $data)
    {
        try {
            ...
            $normalizedAccountingPeriod = $this->normalizeAccountingPeriod($data["accounting_period"] ?? "");
            $yearMonth = date("y-m", strtotime($normalizedAccountingPeriod));
            ...
            VALUES (default, :accounting_period, 'Closed', :busunitcode, 'Active', :usertracker, CURRENT_TIMESTAMP())";
                ...
                $stmt->bindParam(":accounting_period", $normalizedAccountingPeriod, PDO::PARAM_STR);
            ...
            VALUES (default, 'Exinnov', :busunitcode, 'Closing', 'Accounting', :particulars, :id, 'Active', DATE_ADD(NOW(), INTERVAL 8 HOUR))";
            ...
            $logstmt->bindValue(":particulars", $normalizedAccountingPeriod, PDO::PARAM_STR);
            $logstmt->execute();               
    ...
    {   
        try {
                ...
                if(!empty($output['accounting_period']) && $output['accounting_period'] >= $normalizedAccountingPeriod ){
                    $data ="Can't Access";
                }else{
                    $data ="Can Access";
                }
