...

{
...

    {
        ...
        try {
            ...
            $sql = "SELECT count(*) as total FROM tbl_sales_summary WHERE DATE(transdate) = DATE(DATE_ADD(NOW(), INTERVAL 8 HOUR))";
        ...
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            $sequence = $result['total'] + 1; // Correctly accessing the count value



        $formattedSequence = str_pad($sequence, 6, '0', STR_PAD_LEFT);

        $shortUuid = $yearMonthDay . $formattedSequence; // Format: YYYYMMDD000001

            

            // Ensure the generated code is unique

            $checkSql = "SELECT COUNT(*) FROM tbl_sales_summary WHERE sales_id  = :newCode";

            $checkStmt = $this->conn->prepare($checkSql);

            $checkStmt->execute(['newCode' => $shortUuid]);
            ...
            if ($count > 0) {
                ...
                die("Error: Duplicate sales id detected! Try again."); // Stop execution to prevent looping
            ...
            //Credit side find Category on SL Account then charge to revenue 
            ...
            foreach ($data["discountsummary"] as $discount) {
                ...
                $checksldescsql = "SELECT tax_type, category FROM lkp_build_of_products WHERE build_code = :inv_codes AND deletestatus = 'Active'";
                    ...
                    // Concatenate tax_type and category with a hyphen
                     ...
                     //Changed from $sldesc to $discount



                    $description = "SALES - " . $discount['tax_type'] . " - " . $sldesc['category'];
            ...
            //Debit side find Category on SL Account then charge to Discount 803
            ...
            foreach ($data["otherdiscountsummary"] as $otherdiscount) {
                ...
                $negative_amount = round(-1 * (float) $data["transactionsummary"]["total_vat"],2);
            ...
            foreach ($data["salessummary"] as $summary) {
                    ...
                    $description = "SALES - " . $sldesc['tax_type'] . " - " . $sldesc['category'];
...

                {
                    ...
                    $vat = round($summary["total_sales"] / 1.12 * 0.12,2);

                    $negative_amount = round(-1 * $summary["total_sales"] + $vat,2);
...

                {
                    ...
                    $negative_amount = round(-1 * (float) $summary["total_sales"] + (float) $summary["vat"],2);
        ...
        foreach ($category_costs as $category => $total_cost) {
            ...
            if ($slcode) {
                ...
                $stmt->execute();
                    ...
                    VALUES (default, 'Exinnov', :busunitcode, 'salestracker', 'Sales Transaction', :sales_id, :id, 'Active', DATE_ADD(NOW(), INTERVAL 8 HOUR))";
