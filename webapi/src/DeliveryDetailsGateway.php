...

{
...

    {
        ...
        $sql = "SELECT DISTINCT
            ...
            inventory.expirydate AS expiryDate
        ...
        LEFT OUTER JOIN lkp_raw_mats



            ON tbl_delivery_details.inv_code = lkp_raw_mats.mat_code



        LEFT OUTER JOIN tbl_employees AS sender



            ON tbl_delivery_details.sender = sender.empid



        LEFT OUTER JOIN tbl_employees AS receiver



            ON tbl_delivery_details.receiver = receiver.empid
        ...
        LEFT OUTER JOIN lkp_build_of_products
            ...
            ON tbl_delivery_details.inv_code = lkp_build_of_products.build_code



        LEFT OUTER JOIN tbl_delivery_assignment



            ON tbl_delivery_details.del_details_code = tbl_delivery_assignment.del_details_code



        LEFT OUTER JOIN tbl_products_queue_summary



            ON tbl_delivery_assignment.del_code = tbl_products_queue_summary.del_code
        ...
        LEFT OUTER JOIN (
            ...
            SELECT DISTINCT
                ...
                tbl_products_queue_summary.del_code,
            ...
            LEFT OUTER JOIN tbl_delivery_assignment
                ...
                ON tbl_products_queue_summary.del_code = tbl_delivery_assignment.del_code
            ...
            ON tbl_delivery_details.del_details_code = tbl_product_values.del_details_code
        ...
        -- ✅ Join only latest expirydate per pr_queue_code
        ...
        LEFT OUTER JOIN (
            ...
            SELECT pr_queue_code, MAX(expirydate) AS expirydate
            ...
            ON inventory.pr_queue_code = tbl_products_queue_summary.prd_queue_code
...

    {
        ...
        try {
            ...
            if ($data["deliveryitems"][0]["status"] === "Confirmed") {
                ...
                $stmt->bindValue(":status", "For Receiving", PDO::PARAM_STR);
...
$expiry = isset($delivery["expiryDate"]) && $delivery["expiryDate"] !== null
...
$stmt->bindValue(":expirydate", $expiry, PDO::PARAM_STR);
...
$stmt->bindValue(":expirydate", $expiry, PDO::PARAM_STR);
            ...
            foreach ($data["queuedetails"] as $delivery) {
                ...
                // ─── NEW: determine mapped cost_per_uom ────────────────────────────



                // 1. get pricing_category for this busunit (payee)
                ...
                $buStmt->bindValue(':busunitcode', $delivery['payee'], PDO::PARAM_STR);
                ...
                // 2. get cost_per_uom for this inv_code + pricing_category
                ...
                $pdStmt->bindValue(':inv_code', $delivery['inv_code'], PDO::PARAM_STR);
                ...
                // 3. fall back to original if not found
                ...
                $mappedCostPerUom = $pdData['cost_per_uom'] ?? $delivery['cost_per_uom'];
...

    {
        ...
        try {
            ...
            foreach ($data["deliveryitems"] as $delivery) {
                ...
                $stmt->bindValue(":status", "On Delivery", PDO::PARAM_STR);
        ...
        try {
            ...
            if ($data["deliveryitems"][0]["status"] === "Confirmed") {
                ...
                foreach ($data["deliveryitems"] as $delivery) {
                    ...
                    $sql = "SELECT expirydate FROM tbl_inventory_transactions where inv_code = :inv_code and pr_queue_code = :pr_queue_code limit 1";
