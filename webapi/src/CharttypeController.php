    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $charttypecode = $data["charttype_code"];
                $charttypeid = join($charttypecode);

                        $rows = $this->gateway->rejectcharttype($this->user_id,$charttypeid);
                        // echo json_encode(["message" => "Sales Type deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editcharttype($this->user_id,$data);
