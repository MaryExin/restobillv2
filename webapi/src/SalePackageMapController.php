...
class SalePackageMapController
{
    /**
    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $salepackagemapcode = $data["Packagecode"];
                $salepackagemapid = join($salepackagemapcode);

                        $rows = $this->gateway->rejectsalepackagemap($this->user_id,$salepackagemapid);
                        // echo json_encode(["message" => "Product Category deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editsalepackagemap($this->user_id,$data);
