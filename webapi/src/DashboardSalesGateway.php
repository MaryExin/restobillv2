...
class DashboardSalesGateway
{
    ...
    // =========================================================
    // ✅ NEW: HOURLY BI REPORT (used by controller filter=HOURLYBI)
    ...
    // - busunitcode
    //
    // Output: per hour aggregated BI metrics
    // =========================================================
public function getHourlyBIReport($data)
{
    try {
        ...
        if ($dateFrom === "" || $dateTo === "") {
            ...
            return [
                ...
                "errors" => ["datefrom and dateto are required"],
            ];
        }

        // Full-day window for createdtime
        $dtFrom = $dateFrom . " 00:00:00";
        $dtTo   = $dateTo   . " 23:59:59";

        $areaCode    = trim((string)($data["areacode"] ?? ""));
        $busunitcode = trim((string)($data["busunitcode"] ?? ""));

        $sql = "
            SELECT
                DATE(t.createdtime) AS trx_date,
                HOUR(t.createdtime) AS trx_hour,
                CONCAT(LPAD(HOUR(t.createdtime), 2, '0'), ':00') AS hour_label,
                ...
                COUNT(DISTINCT t.sales_trans_id) AS txn_count,
                ROUND(SUM(COALESCE(t.qty, 0)), 2) AS qty_sold,

                ROUND(SUM(COALESCE(t.total_sales, 0)), 2) AS gross_sales,
                ROUND(SUM(COALESCE(t.vat, 0)), 2) AS total_vat,
                ROUND(SUM(COALESCE(t.discount_amount, 0)), 2) AS total_discount,

                ROUND(
                    SUM(COALESCE(t.total_sales, 0))
                    - SUM(COALESCE(t.vat, 0))
                    - SUM(COALESCE(t.discount_amount, 0)),
                2) AS net_sales

            FROM tbl_sales_transactions t
            LEFT JOIN tbl_sales_summary s
                ON t.sales_trans_id = s.sales_trans_id
            LEFT JOIN lkp_busunits bu
                ON s.busunitcode = bu.busunitcode
            LEFT JOIN lkp_area ar
                ON bu.areacode = ar.area_code

            WHERE
                t.deletestatus = 'Active'
                AND (s.deletestatus = 'Active' OR s.deletestatus IS NULL)

                -- Keep your existing transdate window pattern
                AND t.transdate BETWEEN :dateFrom AND :dateTo

                -- Accurate hour buckets based on createdtime
                AND t.createdtime BETWEEN :dtFrom AND :dtTo
        ";

        if ($areaCode !== "") {
            $sql .= " AND bu.areacode LIKE :areacode ";
        }

        if ($busunitcode !== "") {
            $sql .= " AND s.busunitcode LIKE :busunitcode ";
        }
        ...
        $sql .= "
            GROUP BY
                ...
                ar.area_name
            ORDER BY
                DATE(t.createdtime) ASC,
                HOUR(t.createdtime) ASC,
                bu.name ASC
        ";
        ...
        // ✅ Recommended production behavior: log real error, return generic msg
        error_log("HourlyBI error: " . $e->getMessage());
    ...
    public function getAllOpenStoresToday($data)
    {
        try {
            $sql = "SELECT
                ...
                CASE
                    WHEN tbl_cash_sales_summary_tracker.opening_time <> 0
                         AND tbl_cash_sales_summary_tracker.closing_time = 0
                    THEN 'OPEN'
                    ELSE 'CLOSED'
                END AS storestatus
            FROM lkp_busunits
            LEFT OUTER JOIN tbl_cash_sales_summary_tracker
                ON lkp_busunits.busunitcode = tbl_cash_sales_summary_tracker.busunitcode
            LEFT OUTER JOIN tbl_employees
                ON tbl_cash_sales_summary_tracker.usertracker = tbl_employees.empid
            WHERE
...
public function getOtherMOPByDateRange(array $data): array
{
    try {
        $dateFrom = trim((string)($data["datefrom"] ?? ""));
        $dateTo   = trim((string)($data["dateto"] ?? ""));
        ...
        if ($dateFrom === "" || $dateTo === "") {
            http_response_code(422);
            return ["message" => "missingDateRange"];
        }

        $dfOk = \DateTime::createFromFormat("Y-m-d", $dateFrom) !== false;
        $dtOk = \DateTime::createFromFormat("Y-m-d", $dateTo) !== false;

        if (!$dfOk || !$dtOk) {
            http_response_code(422);
            return ["message" => "invalidDateFormat"]; // expects YYYY-MM-DD
        }

        $areaCode = trim((string)($data["areacode"] ?? ""));
        $busunit  = trim((string)($data["busunitcode"] ?? ""));

        // ✅ Cash MOP id to exclude
      $cashMopId = trim((string)($data["cashmopid"] ?? "MP-60b729686a4c"));


        $sql = "
            SELECT
                m.mop_id,
                UPPER(COALESCE(lm.description, 'UNKNOWN')) AS description,
                ROUND(SUM(COALESCE(m.amount, 0)), 2) AS amount
            FROM tbl_mop_summary m
            LEFT JOIN lkp_mop lm
                ON m.mop_id = lm.mop_id
            LEFT JOIN tbl_sales_summary s
                ON m.mop_trans_id = s.mop_trans_id
            LEFT JOIN lkp_busunits bu
                ON s.busunitcode = bu.busunitcode
            WHERE
                m.deletestatus = 'Active'
                AND (lm.deletestatus = 'Active' OR lm.deletestatus IS NULL)
                AND (s.deletestatus = 'Active' OR s.deletestatus IS NULL)
                AND m.transdate BETWEEN :dateFrom AND :dateTo
        ";
        ...
        if ($busunit !== "") {
            $sql .= " AND s.busunitcode LIKE :busunitcode ";
            $params[":busunitcode"] = "%" . $busunit . "%";
        }

        $sql .= "
            GROUP BY
                m.mop_id,
                UPPER(COALESCE(lm.description, 'UNKNOWN'))
            ORDER BY
                amount DESC
        ";
        ...
        error_log('OtherMOP error: ' . $e->getMessage());
        http_response_code(500);
