...
class TicketController
{
    ...
    public function processInfiniteRequest(string $method, $page, $pageIndex, $pageData, $search,$busunitcode,$status): void
    {
        if (true) {
            if ($method == "GET") {
                echo json_encode($this->gateway->getInfiniteData($this->user_id,$page, $pageIndex, $pageData, $search,$busunitcode,$status));
            } elseif ($method == "POST") {
        ...
        if (true) {
            if ($method == "GET") {
                ...
                if($data["process"] == "status"){
                     $this->gateway->ticketstatus($this->user_id, $data);
                }
                elseif($data["process"] == "remarks"){
                     $this->gateway->ticketremarks($this->user_id, $data);
                }elseif($data["process"] == "date"){
                     $this->gateway->ticketEstimatedtime($this->user_id, $data);
                }
                elseif($data["process"] == "evaluation"){
                     $this->gateway->ticketEvaluation($this->user_id, $data);
                }elseif($data["process"] == "message"){
                     $this->gateway->ticketMessage($this->user_id, $data);
                }else{
                     $this->gateway->ticketdelete($this->user_id, $data);
                }          
            } else {
                $this->respondMethodNotAllowed("GET, POST");
            }
    ...
    public function processClearingacknowledge(string $method, $data): void
    {
        if (true) {
            if ($method == "GET") {
                echo json_encode($this->gateway->getClearingData());
            } elseif ($method == "POST") {
                $this->gateway->ticketacknowledge($this->user_id, $data);
            } else {
                $this->respondMethodNotAllowed("GET, POST");
            }
        } else {
    ...
    public function processReadRequest(string $method,$data): void
    {
        if (true) {
            if ($method == "GET") {
                echo json_encode($this->gateway->getAllData());
            } elseif ($method == "POST") {
                if(isset($data["checkedData"])){
                    echo json_encode($this->gateway->pettycashForReplenish($data));
                }elseif(isset($data['othermaprefroute'])){
                    echo json_encode($this->gateway->getGroupData($this->user_id, $data));
                }
                else{
                    echo json_encode($this->gateway->getthepettycashSummaryDetails($data));
                }
                ;
            } else {
                $this->respondMethodNotAllowed("GET, POST");
