...
class PcfsetupController
{
    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $pcfcode = $data["pcfcodes"];
                // $pcfid = join($pcfcode);

                        $rows = $this->gateway->rejectpcfs($this->user_id,$pcfcode);
                        echo json_encode(["message" => "pcf deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editpcf($this->user_id,$data);
                        echo json_encode(["message" => "pcf Edit", "rows" => $rows]);
                }
            } else {
            ...
            switch ($method) {
                ...
                case "PATCH":
                        $pcfcode = $data["brand_code"];

                        $rows = $this->gateway->rejectpcfs($data);
    ...
    public function processReadinfiniteRequest(string $method, $page, $pageIndex, $pageData, $search): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                echo json_encode($this->gateway->getbyPageData($page, $pageIndex, $pageData, $search));
