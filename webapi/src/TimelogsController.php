...
class TimelogsController
{
    ...
    public function processRequest(string $method, $data)
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                if (array_key_exists("Add", $data)) {
                    ...
                    $rows = $this->gateway->AddNewLogs($this->user_id, $data);

                    // echo json_encode(["message" => "timelogs deleted", "rows" => $rows]);
                ...
                elseif (!array_key_exists("edit", $data)) {
                    ...
                    $rows = $this->gateway->rejecttimelogss($this->user_id, $data);
                    ...
                    $rows = $this->gateway->edittimelogs($this->user_id, $data);

                    // echo json_encode(["message" => "timelogs Edit", "rows" => $rows]);
            ...
            switch ($method) {
                ...
                case "PATCH":
                    ...
                    $timelogscode = $data["brand_code"];

                    $rows = $this->gateway->rejecttimelogss($data);
    ...
    public function processReadRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                echo json_encode($this->gateway->getbyPageData($data));
