...
class BuildGateway
{
    ...
    public function createForUser($user_id, $data)
    {
        ...
        try {
            ...
            $insertSQL = "INSERT INTO lkp_build_of_products ()
                          ...
                          'Active', :user_tracker, DATE_ADD(NOW(), INTERVAL 8 HOUR),:brandcode,:productcode)";
            ...
            $stmt->bindValue(":productcode", strtoupper($productcode));
            $stmt->execute();
    ...
    public function getInfiniteReadData($page, $pageIndex, $pageData, $search)
    {
        ...
        $sql = "SELECT
                    lkp_build_of_products.seq, 
                    lkp_build_of_products.build_code,
                    lkp_build_of_products.desc,
                    lkp_build_of_products.build_qty,
                    lkp_build_of_products.uomval,
                    lkp_build_of_products.uom,
                    lkp_build_of_products.seq,
                    ...
                    lkp_build_of_products.tax_type,
                    lkp_build_of_products.category,
                    lkp_build_of_products.expiry_days,
                    lkp_build_of_products.portion_parent,
                    lkp_build_of_products.brandcode,
                    lkp_product_brand.brand_desc,
                    lkp_build_of_products.productcode,
                    lkp_build_of_products.deletestatus
                FROM
                    lkp_build_of_products
                        LEFT OUTER JOIN
                    tbl_pricing_details ON lkp_build_of_products.build_code = tbl_pricing_details.inv_code
                        LEFT OUTER JOIN
                    lkp_pricing_code ON tbl_pricing_details.pricing_code = lkp_pricing_code.uuid
                        LEFT OUTER JOIN
                    lkp_product_brand ON lkp_build_of_products.brandcode = lkp_product_brand.brandcode
                WHERE
                    lkp_build_of_products.deletestatus = 'Active'
                AND 
            (lkp_build_of_products.desc LIKE CONCAT('%', :search, '%') OR lkp_build_of_products.productcode LIKE CONCAT('%', :codesearch, '%'))
