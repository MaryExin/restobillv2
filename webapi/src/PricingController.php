...
class PricingController
{
 ...
 public function processRequest(string $method, $data): void
    {
        // ✅ analytics uses GET + query params
        if ($method === "GET") {
            $action = $_GET["Action"] ?? "";
            if ($action === "analytics") {
                echo json_encode($this->gateway->getAnalytics($_GET));
                return;
            }

            // default existing list
        ...
        if ($method === "POST") {
            ...
            // ✅ batch upload
            if ($action === "multiadd") {
                $rows = $this->gateway->createForMultiProduce($this->user_id, $data);
                echo json_encode($rows);
                return;
            }

            // ✅ single create (history preserved)
        ...
        if ($method === "PATCH") {
            // existing behavior:
            // - if no "edit" => delete (inactive)
            if (!array_key_exists("edit", $data)) {
                $this->gateway->rejectpricing($this->user_id, $data);
                return;
            }

            // ✅ edit now becomes "history write" (inactivate old + insert new)
            $this->gateway->editpricing($this->user_id, $data);
            return;
        }
