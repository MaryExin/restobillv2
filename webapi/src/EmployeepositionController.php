    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $departmentcode = $data["emppositioncode"];
                $departmentid = join($departmentcode);

                        $rows = $this->gateway->rejectdepartments($this->user_id,$departmentid);
                        // echo json_encode(["message" => "Department deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editdepartment($this->user_id,$data);
                        // echo json_encode(["message" => "Department Edit", "rows" => $rows]);
                }
            } else {
