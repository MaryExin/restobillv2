...
$sldata = [
  [
    'glcodes' => '700',
    'descrip' => 'SALES - VATABLE - ',
  ],
  [
    'glcodes' => '701',
    'descrip' => 'SALES - VAT EXEMPT - ',
  ],
  [
    'glcodes' => '703',
    'descrip' => 'SALES - ZERO RATED - ',
  ],
  [
    'glcodes' => '800',
  ...
  [
    ...
    $slcode = $datas['glcodes'] . sprintf("%03d", $uniqueNumber);
    ...
    $acctgslcode = $datas['glcodes'] . sprintf("%03d", $uniqueNumber);
    $acctggldecs = substr($datas['descrip'], 0, -3);
    $acctgsldesc = $datas['descrip'] . $data["productcatdesc"];

    $charttypedatas = $data['charttypedata'];

    foreach ($charttypedatas as $charttype){
    $acctgaccountstmt->bindValue(":charttype", $charttype, PDO::PARAM_STR);
    $acctgaccountstmt->bindValue(":acctgglcodes", $datas['glcodes'], PDO::PARAM_STR);
    $acctgaccountstmt->bindValue(":acctggldecs", $acctggldecs, PDO::PARAM_STR);
    $acctgaccountstmt->bindValue(":acctgslcodes", $acctgslcode, PDO::PARAM_STR);
    $acctgaccountstmt->bindValue(":acctgsldescs", $acctgsldesc, PDO::PARAM_STR);
    $acctgaccountstmt->bindValue(":acctguser_tracker", $user_id, PDO::PARAM_STR);
    $acctgaccountstmt->execute();
    }
    }

        $sql = "INSERT INTO lkp_product_category () VALUES (default, :category_id, :productcatdesc, :sort, :showhidesaless, 'Active', :user_tracker, DATE_ADD(NOW(), INTERVAL 8 HOUR))";
        $stmt = $this->conn->prepare($sql);
        ...
        $stmt->bindValue(":showhidesaless", $data["showhidesales"], PDO::PARAM_STR);
        $stmt->bindValue(":user_tracker", $user_id, PDO::PARAM_STR);
        $stmt->execute();
    ...
    $checkforduplisql = "SELECT COUNT(*) as count FROM lkp_product_category WHERE category = :productcatdesc && sort = :sorts && showhide_sales = :showhide_saless && deletestatus = 'Active'";
    $checkforduplistmt = $this->conn->prepare($checkforduplisql);
    ...
    $checkforduplistmt->bindValue(":showhide_saless", $id["showhidesales"], PDO::PARAM_STR);
    $checkforduplistmt->execute();
    $rowCount = $checkforduplistmt->fetchColumn();
    ...
    if($rowCount > 0) {
        ...
        $stmt->bindValue(":showhidesaless", $id["showhidesales"], PDO::PARAM_STR);
        $stmt->bindValue(":usertracker", $user_id, PDO::PARAM_STR);
        $stmt->bindValue(":id", $id["productcatcode"], PDO::PARAM_STR);
        $stmt->execute();

        $sql = "UPDATE lkp_slcodes
        SET sldescription = REPLACE(sldescription, :prevdescs, :newdesc)
        WHERE sldescription LIKE CONCAT('%', :prevdesc, '%')
        AND glcode IN ('140', '700', '701', '703', '800')";
        
        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":newdesc", $id["productcatdesc"], PDO::PARAM_STR);
        $stmt->bindValue(":prevdescs", $id["prevproductcatdesc"], PDO::PARAM_STR);       // e.g. 'FOODS'
        $stmt->bindValue(":prevdesc", $id["prevproductcatdesc"], PDO::PARAM_STR);  // e.g. 'FOOD'
        $stmt->execute();

        $sql = "UPDATE lkp_chart_of_accounts
        SET sl_description = REPLACE(sl_description, :prevdescss, :newdescs)
        WHERE sl_description LIKE CONCAT('%', :prevdescs, '%')
        AND glcode IN ('140', '700', '701', '703', '800')";
        
        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":newdescs", $id["productcatdesc"], PDO::PARAM_STR);
        $stmt->bindValue(":prevdescss", $id["prevproductcatdesc"], PDO::PARAM_STR);       // e.g. 'FOODS'
        $stmt->bindValue(":prevdescs", $id["prevproductcatdesc"], PDO::PARAM_STR);  // e.g. 'FOOD'
        $stmt->execute();
        // return $stmt->rowCount();
