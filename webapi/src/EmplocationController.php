    ...
    public function processRequest(string $method, $data): void
    {
        ...
        if (true) {
            ...
            if ($method == "GET") {
                ...
                $saletypecode = $data["emplocation_code"];
                $saletypeid = join($saletypecode);

                        $rows = $this->gateway->rejectsaletype($this->user_id,$saletypeid);
                        // echo json_encode(["message" => "Sales Type deleted", "rows" => $rows]);

                exit;
                } else{
                        $rows = $this->gateway->editsaletype($this->user_id,$data);
