...
class FixedAssetGateway
{
    ...
    private function extractReferenceParts($reference, $prefix)
    {
        ...
        if (strpos($normalizedReference, $normalizedPrefix) !== 0) {
            if (!preg_match('/^[A-Z]{2}-\d{10}$/', $normalizedReference)) {
                throw new Exception("Invalid transaction reference format.");
            }
        }

        $suffix = preg_replace('/^[A-Z]{2}-/', '', $normalizedReference);
    ...
    public function getDisbursementVoucherDetails($data)
    {
        ...
        $sql = "SELECT 
                ...
                ORDER BY 
                    CASE 
                        WHEN tbl_accounting_transactions.supplier_code IS NOT NULL
                             AND tbl_accounting_transactions.supplier_code <> '' THEN 0
                        ELSE 1
                    END,
                    CASE 
                        WHEN tbl_accounting_transactions.amount < 0 THEN 0
                        ELSE 1
                    END,
                    tbl_accounting_transactions.seq ASC
                LIMIT 1";
        ...
        $sql = "SELECT DISTINCT
                ...
                FROM
                    tbl_accounting_transactions
                        ...
                        LEFT OUTER JOIN tbl_fixed_assets
                            ON tbl_accounting_transactions.menutransactedref = tbl_fixed_assets.menutransactedref
                            AND tbl_fixed_assets.deletestatus = 'Active'
                WHERE
                    tbl_accounting_transactions.menutransactedref = :disbursementid
                    AND tbl_accounting_transactions.deletestatus = 'Active'
    ...
    public function insertEntries($user_id, $data)
    {
        ...
        $this->conn->exec("SET time_zone = 'Asia/Manila'");
            ...
            if ($isEditing) {
                ...
                $parts = $this->extractReferenceParts($editReference, "FA");
                $id = $parts["id"];
                $year = $parts["year"];
                ...
                $deleteMapSql = "DELETE FROM lkp_acctg_transactions_map
                                 WHERE transactionid = :transaction_id_delete";
                $deleteMapStmt = $this->conn->prepare($deleteMapSql);
                $deleteMapStmt->bindValue(":transaction_id_delete", $editReference, PDO::PARAM_STR);
                ...
                $deleteDisbursementStmt->bindValue(":disbursement_reference_delete", $uuidDB, PDO::PARAM_STR);
                $deleteDisbursementStmt->execute();

                $deleteFixedAssetSql = "DELETE FROM tbl_fixed_assets
                                        WHERE fixedassetid = :fixedasset_reference_delete
                                           OR menutransactedref = :menu_reference_delete";
                $deleteFixedAssetStmt = $this->conn->prepare($deleteFixedAssetSql);
                $deleteFixedAssetStmt->bindValue(":fixedasset_reference_delete", $editReference, PDO::PARAM_STR);
                $deleteFixedAssetStmt->bindValue(":menu_reference_delete", $editReference, PDO::PARAM_STR);
                $deleteFixedAssetStmt->execute();
      ...
      if($Entries["transactionType"] == "Purchases")
      {
        ...
        VALUES (default, 'Exinnov', :busunitcode, 'FixedAssets', 'FixedAssets', :particulars, :id, 'Active', DATE_ADD(NOW(), INTERVAL 8 HOUR))";
